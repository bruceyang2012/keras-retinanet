...
def _compute_ap(recall, precision, voc_07_metric=False):
    if voc_07_metric:
        # 11 point metric
        ap = 0.
...
def _get_detections(generator, model, score_threshold=0.05, max_detections=100):
    ...
    for i in range(generator.size()):
        ...
        # copy to detections to all_detections
...
def _get_annotations(generator):
    ...
    for i in range(generator.size()):
        ...
        # copy to detections to all_annotations
        for label in range(generator.num_classes()):
    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(generator.size()):
            ...
            for d in detections:
                ...
                assigned_anchor = np.argmax(overlaps, axis=1)
                max_overlap     = overlaps[0, assigned_anchor]

                if max_overlap > iou_threshold:
                    #cv2.rectangle(image, (int(annotations[assigned_anchor, 0]), int(annotations[assigned_anchor, 1])), (int(annotations[assigned_anchor, 2]), int(annotations[assigned_anchor, 3])), (0, 255, 0), 3)
                    #cv2.rectangle(image, (int(d[0]), int(d[1])), (int(d[2]), int(d[3])), (255, 255, 0), 3)
                    #cv2.imshow("image", image)
                    #cv2.waitKey()

                    false_positives.append(0)
                    true_positives.append(1)
                    annotations = np.delete(annotations, assigned_anchor, axis=0)
                else:
                    false_positives.append(1)
                    true_positives.append(0)
        ...
        average_precision  = _compute_ap(recall, precision, voc_07_metric=False)
